...
class MapStore:
    ...
    def __init__(self) -> None:
        ...
        # device_type → ISO timestamp последнего обновления
    ...
    def update(self, device_type: str, registers: dict[str, dict]) -> None:
        ...
        self._updated_at[device_type] = datetime.now(timezone.utc).isoformat()
        logger.info(
    ...
    def stats(self) -> dict:
        """Сводка состояния MapStore для диагностики."""
        return {
            device_type: {
                "register_count": len(regs),
                "updated_at": self._updated_at.get(device_type),
            }
            for device_type, regs in self._maps.items()
        }
